...
使用 ANSI 转义码全屏刷新，单键驱动交互。
遵循 LOGIC.md 分支：清除屏幕 → 渲染完整帧 → 读取一键 → 循环。
"""
import os
import subprocess
import sys
...
def render(state: ConfirmState) -> str:
    ...
    if not state.is_done:
        lines.append(f"  {BOLD}[a]{RESET} 接受  {BOLD}[e]{RESET} 编辑  {BOLD}[r]{RESET} 拒绝  {BOLD}[q]{RESET} 退出")
    else:
...
def _render_suggestion(state, w: int) -> list[str]:
    """渲染当前建议的详细信息。"""
    lines = []
    s = state.current
    if not s:
    ...
    lines.append(f"  {BOLD}📋 建议:{RESET} {s.summary}")
...
def _render_editing(state, w: int) -> list[str]:
    """编辑模式的提示。"""
    lines = []
    cur = state.current
    if not cur:
...
def _render_summary(state, w: int) -> list[str]:
    """最终汇总。"""
    lines = []
...
def run(state: ConfirmState) -> None:
    """主循环。"""
    while True:
